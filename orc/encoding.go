		...
		case Encoding_DIRECT: // numbers encoding in big-endian
			...
			for i := 0; i < length; {
				...
				case 3:
					...
					j := prevj
					for ; j < 16; j += 3 {
						x = uint64((data >> byte(15-j)) & 0x7)
					}
					prevj = j*/
					x, err := rle.readBits(in, 3)
					if err != nil {
						return err
					}
					if rle.signed {
